...
def FollowModel_1(source_model):
    ...
    # 变质模型
    follow_model = Conv2d(in_channels=source_model.in_channels, out_channels=source_model.out_channels,
                          kernel_size=source_model.kernel_size, pad_mode=source_model.pad_mode,
                          has_bias=source_model.has_bias, data_format=source_model.data_format,
...
def FollowModel_3(source_model):
    ...
    follow_model = Conv2d(in_channels=source_model.in_channels, out_channels=source_model.out_channels,
                          kernel_size=source_model.kernel_size, pad_mode=source_model.pad_mode,
                          has_bias=source_model.has_bias, data_format=source_model.data_format,
                          weight_init=Tensor(source_model.weight).transpose(0,1,3,2) if FORMAT== "NCHW"
                          else Tensor(source_model.weight).transpose(0,2,1,3),
                          bias_init=Tensor(source_model.bias))
    bias = source_model.parameters_dict()["bias"]
        ...
        for muta_mode in range(0, 4):
            for i in range(100):
                ...
                if muta_mode == 0:
                    ...
                    source_result = (source_model(data) - bias) * Tensor(delta)
                    ...
                    bias = bias.reshape(-1, 1, 1) if FORMAT == "NCHW" else bias
                    if FORMAT == "NCHW":
                        source_result = (source_model(data) - bias).transpose(0, 1, 3, 2)
