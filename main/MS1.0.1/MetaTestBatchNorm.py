...
def FollowModel_4(source_model):
    ...
    follow_model = BatchNorm2d(num_features=source_model.num_features,
                               eps=source_model.eps,
                               gamma_init=Tensor(source_model.parameters_dict()["gamma"]) * delta,
                               beta_init=Tensor(source_model.parameters_dict()["beta"]),
                               moving_mean_init=Tensor(source_model.parameters_dict()['mean']),
                               moving_var_init=Tensor(source_model.parameters_dict()["variance"]))
        ...
        for muta_mode in range(0, 4):
            for i in range(100):
                ...
                if muta_mode == 0:
                    ...
                    beta = Tensor(beta.asnumpy().reshape(-1, 1, 1)) if FORMAT == "NCHW" else beta
