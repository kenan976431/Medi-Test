        ...
        for muta_mode in range(0, 3):
            for i in range(100):
                ...
                if muta_mode == 0:
                    ...
                    source_result = avg_pool(data, kernel=[pool_size, pool_size], stride=[strides, strides]) * delta
                    follow_result = avg_pool(data * delta, kernel=[pool_size, pool_size], stride=[strides, strides])
                    ...
                    delta = 0
                    # 转置 NCHW
                    source_result = avg_pool(data, kernel=[pool_size, pool_size], stride=[strides, strides])
                    source_result = np.transpose(source_result, (0, 1, 3, 2))
                    follow_result = avg_pool(data.transpose((0, 1, 3, 2)),
